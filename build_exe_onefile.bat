...
python -m PyInstaller --noconfirm --clean --onefile --windowed --name "%APP_NAME%" --icon "%~dp0assets\app.ico" --distpath "dist\%APP_NAME%_单文件" --workpath "build\%APP_NAME%_onefile" --specpath "build\%APP_NAME%_onefile" main.py
if errorlevel 1 (
    echo 打包失败!
    pause
    exit /b 1
