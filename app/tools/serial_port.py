...
def generate_stylesheet(theme_name=None):
    ...
    # 根据主题风格调整圆角大小
    is_windows = theme_name and ("Windows" in theme_name or theme_name == "Windows 11")
    border_radius = 4 if is_windows else 16  # Windows用小圆角，其他用大圆角
...
class SerialDebugTool(QMainWindow):
    def __init__(self):
        ...
        # 记录当前是否为 Windows 风格
        self._is_windows_style = "Windows" in ThemeManager.current_theme
    ...
    def _setup_ui(self):
        ...
        theme = ThemeManager.get_theme()
        is_windows = "Windows" in ThemeManager.current_theme
        
        if is_windows:
            # Windows 风格：标准窗口，有标题栏和边框
    ...
    def _switch_to_theme(self, theme_name):
        ...
        # 检查是否需要重建窗口
        is_windows = "Windows" in theme_name
        was_windows = hasattr(self, '_is_windows_style') and self._is_windows_style
        
        if is_windows != was_windows:
            # 风格改变了，需要重启应用以应用新的窗口样式
            self._is_windows_style = is_windows
            QMessageBox.information(
                self, 
                "主题已切换", 
                f"已切换到 {ThemeManager.THEMES[theme_name]['name']} 主题。\n\n注意：窗口样式已改变，建议重启应用以获得最佳效果。"
            )
        else:
            # 刷新标题栏（仅 macOS 风格）
            if not is_windows:
